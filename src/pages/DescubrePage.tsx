...
const categoryOptions = ['Todas las categorías', 'Música', 'Conferencia', 'Concierto', 'Deportes', 'Arte', 'Gastronomía', 'Teatro', 'Comedia', 'Jazz'];
...
export default function DescubrePage() {
  ...
  const filtered = mergedEvents.filter((ev) => {
    const matchSearch = search === '' || ev.title.toLowerCase().includes(search.toLowerCase()) || ev.city.toLowerCase().includes(search.toLowerCase());
    const matchCity = cityFilter === 'Todas las ciudades' || ev.city === cityFilter;
    const matchCategory = categoryFilter === 'Todas las categorías' || ev.badge === categoryFilter || ev.category === categoryFilter;
    const matchPrice =
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-8 py-10">
        {/* Title */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-1">Descubre Eventos</h1>
        ...
        <div className="flex flex-wrap items-center gap-2.5 mb-6">
          ...
          <FilterSelect value={dateFilter} options={dateOptions} onChange={setDateFilter} />
          <FilterSelect value={cityFilter} options={cityOptions} onChange={setCityFilter} />
          <FilterSelect value={categoryFilter} options={categoryOptions} onChange={setCategoryFilter} />
          <div className="flex items-center gap-1.5 text-gray-500 ml-1">
            <span className="text-sm font-medium">Precio:</span>
          </div>
          <FilterSelect value={priceFilter} options={priceOptions} onChange={setPriceFilter} />
        </div>

        {/* Results bar */}
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-gray-500">
            Mostrando <span className="font-semibold text-gray-800">{filtered.length}</span> eventos
          </p>
